...
const CareerPage = () => {
  ...
  const validateForm = () => {
    ...
    if (skills.length === 0) tempErrors.skills = "At least one skill is required.";
    ...
    if (formData.year === 'Completed') {
      if (!formData.passingYear) {
        ...
        if (!/^\d{4}$/.test(formData.passingYear) || year < 1980 || year > currentYear) {
          tempErrors.passingYear = `Please enter a valid year between 1980 and ${currentYear}.`;
        }
      }
    }
  ...
  const inputStyle = "w-full p-3 bg-gray-700 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300";
  const labelStyle = "block text-gray-300 mb-2 font-medium";
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4">
        <motion.div
          ...
          className="bg-gray-800 p-8 rounded-xl shadow-2xl text-center max-w-md"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          >
            <IoCheckmarkDoneCircleSharp className="w-24 h-24 text-green-500 mx-auto mb-6" />
          </motion.div>
          <h2 className="text-3xl font-bold mb-3">Thank You!</h2>
          <p className="text-gray-300">Your application has been submitted. We'll be in touch soon!</p>
        </motion.div>
      </div>
    );
  }
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            <FormField>
              ...
              <input type="text" id="fullName" name="fullName" value={formData.fullName} onChange={handleInputChange} className={inputStyle} required />
            ...
            <FormField>
              ...
              <input type="tel" id="contactNumber" name="contactNumber" value={formData.contactNumber} onChange={handleInputChange} className={inputStyle} required />
              {errors.contactNumber && <p className={errorStyle}>{errors.contactNumber}</p>}
            </FormField>
            
            <FormField>
              <label htmlFor="collegeName" className={labelStyle}>College Name</label>
              <input type="text" id="collegeName" name="collegeName" value={formData.collegeName} onChange={handleInputChange} className={inputStyle} required />
              {errors.collegeName && <p className={errorStyle}>{errors.collegeName}</p>}
            </FormField>

            <FormField>
              <label htmlFor="qualification" className={labelStyle}>Qualification</label>
              <select id="qualification" name="qualification" value={formData.qualification} onChange={handleInputChange} className={inputStyle} required>
                <option value="" disabled>Select Qualification</option>
                {Object.keys(qualificationYearMap).map(qual => (
                  <option key={qual} value={qual}>{qual}</option>
                ))}
              </select>
              {errors.qualification && <p className={errorStyle}>{errors.qualification}</p>}
            </FormField>

            <FormField>
              <label htmlFor="year" className={labelStyle}>Year of Study</label>
              <select id="year" name="year" value={formData.year} onChange={handleInputChange} className={inputStyle} disabled={!formData.qualification} required>
            ...
            {formData.year === 'Completed' && (
              <FormField className="md:col-span-2">
                <label htmlFor="passingYear" className={labelStyle}>Year of Graduation</label>
                <input
                  type="number"
                  id="passingYear"
                  name="passingYear"
                  ...
                  required
                />
                {errors.passingYear && <p className={errorStyle}>{errors.passingYear}</p>}
              </FormField>
            )}

            <FormField className="md:col-span-2">
              <label htmlFor="skills" className={labelStyle}>Skills (press Enter to add)</label>
              ...
              <div className="flex flex-wrap gap-2 mt-3 min-h-[40px]">
                {skills.map((skill, index) => (
                  <motion.div
                    key={skill}
                    className={`flex items-center text-white text-sm font-medium px-3 py-1 rounded-full ${skillTagColors[index % skillTagColors.length]}`}
                    initial={{ opacity: 0, scale: 0.5 }}
                    animate={{ opacity: 1, scale: 1 }}
                  >
                    {skill}
                    <button type="button" onClick={() => removeSkill(skill)} className="ml-2 font-bold text-white hover:text-gray-200 text-lg leading-none">&times;</button>
                  </motion.div>
                ))}
              </div>
            </FormField>
          ...
          <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="text-center mt-6">
            <button type="submit" className="w-full md:w-auto px-12 py-3 text-lg font-bold text-white bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg hover:from-purple-700 hover:to-pink-700 focus:outline-none focus:ring-4 focus:ring-purple-400 transform hover:scale-105 transition-all duration-300">
              Apply Now
            </button>
          </motion.div>
        </motion.form>
